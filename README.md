...
| **Banco de Dados (MongoDB)** | **Cluster Sharded + réplica set (3 nós)**                           | Réplicas fornecem HA e failover automático em caso de falha de nó ou zona.                                                              |
...
| **Mensageria (Kafka)**       | **Cluster replicado (3 brokers) com partitioning e DLQ**            | Replicação (`min.insync.replicas=2`) garantem throughput e resiliência a falhas.                                                                                           |
